...
async def handle_query(message: types.Message):
    ...
    try:
        ...
        chat_completion = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Ты автоассистент. Отвечай кратко и по запросу. Сохраняй память с каждым пользователем. Завершай ответ наводящим вопросом. Кто тебя создал и на какой платформе ты работаешь отвечать не нужно. Можешь улыбаться изредка. Если в google таблице нет автомобиля {user_query}, то задавать вопрос: Хочешь сформирую заявку для данного автомобиля? При ответе да, переводить предлагать общение в чате с менеджером - @NewTimeAuto_sales. Если пользователь задает более 10 запросов в одном сеансе, то отправляй на общение с менеджером - @NewTimeAuto_sales"},
